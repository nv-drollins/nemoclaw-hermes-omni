#!/usr/bin/env bash
# Ask for sudo once when the install path needs host-level checks.
set -euo pipefail

if ! command -v sudo >/dev/null 2>&1; then
    echo "sudo is required for first-time host setup and NemoClaw preflight checks." >&2
    exit 1
fi
...
echo "This first-time setup needs sudo for host package checks and NemoClaw preflight."
